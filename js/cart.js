  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function refreshSummary() {
      ...
      document.querySelectorAll('.cart-item').forEach(item => {
        ...
        const line= parseInt(item.querySelector('.cart-line-total')
                               .textContent.replace(/\D/g,''), 10) || 0;
        if (cb.checked) {
          totalQty   += qty;
          totalPrice += line;
        }
      ...
      if (checkAllEl) {
        const all     = Array.from(document.querySelectorAll('.cart-checkbox'));
...
function updateQty(itemId, newQty) {
    ...
    .then(data => {
        if (data.status === 'ok') {
            ...
            if (itemEl) {
                const lineTotalElement = itemEl.querySelector('.cart-line-total');
                if (lineTotalElement) {
                    lineTotalElement.textContent = data.line_total;
                } else {
                    console.error('Không tìm thấy phần tử .cart-line-total');
                }
...
document.querySelector('.qty-increase').addEventListener('click', function(e) {
  ...
  document.querySelectorAll('.cart-item').forEach(item => {
    ...
    const qty  = parseInt(item.querySelector('.cart-qty-input').value, 10) || 0;
    const line = parseInt(
      item.querySelector('.cart-line-total').textContent.replace(/\D/g, ''),
      10
    ) || 0;
  ...
  if (allCb) allCb.checked = itemsCb.length > 0 && checkedCb.length === itemsCb.length;
}
