  ...
  <div class="search-info">
    ...
    <div class="search-actions">
      <div class="qty-control" id="cart-controls-${item.id}">
        <button onclick="handleIncrease(${item.id}, -1, ${item.stockQty})">
        ...
        <input
          ...
          max="${item.stockQty}"
          value="${cartQty}"
          oninput="handleQtyChange(${item.id}, ${item.stockQty})"
        />
        <button onclick="handleIncrease(${item.id}, 1, ${item.stockQty})">
          <i class="fa-solid fa-plus"></i>
        </button>
        <button class="cart-btn" onclick="event.stopPropagation(); if(window.isLoggedIn){addToCart(${item.id}, document.getElementById('qty-input-${item.id}').value)}else{alert('Vui lòng đăng nhập để thêm vào giỏ!');}">
          <i class="fa-solid fa-bag-shopping"></i>
        </button>
        <button class="favorite-btn" data-item-id="${item.id}" title="Thêm vào yêu thích" onclick="event.stopPropagation(); window.favoritesManager && window.favoritesManager.toggleFavorite(${item.id});">
